...
import { Sparkles, RefreshCw, Maximize2, Minimize2, Settings } from "lucide-react";
...
const WorkspaceEditor = ({
  ...
  return (
    <div className="modern-card !p-0 !flex-row !overflow-hidden !border-y-0 !border-l-0 !border-r !border-border !bg-bg-secondary !rounded-none w-full h-full shrink-0">
      <div className="grow flex flex-col min-w-0">
        ...
        {/* Breadcrumb Bar */}
        <div className="h-[22px] bg-bg-secondary border-b border-border flex justify-between items-center px-4 text-[0.68rem] text-text-secondary select-none">
        ...
        <div className="editor-wrapper !border-none !rounded-none grow">
          {diffView ? (
            <DiffEditor height="100%" language={getMonacoLanguage()} original={getOriginalCode()} modified={getActiveCode()} onMount={handleDiffMount} theme="light" options={{ fontSize, fontFamily: "var(--font-family-code)", minimap: { enabled: false }, wordWrap, readOnly: false, automaticLayout: true, renderSideBySide: true, linkedEditing: true, renameOnType: true, tabCompletion: "on" }} />
          ) : (
            <Editor height="100%" language={getMonacoLanguage()} value={getActiveCode()} onChange={handleEditorChange} onMount={handleEditorDidMount} theme="light" options={{ fontSize, fontFamily: "var(--font-family-code)", minimap: { enabled: minimap }, wordWrap, lineNumbers: "on", readOnly: false, automaticLayout: true, padding: { top: 12, bottom: 12 }, tabSize, scrollBeyondLastLine: false, bracketPairColorization: { enabled: true }, linkedEditing: true, renameOnType: true, tabCompletion: "on" }} />
          )}
        </div>
      </div>
    </div>
